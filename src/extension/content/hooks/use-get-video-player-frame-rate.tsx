 ...
 */
export const useGetVideoPlayerFrameRate = (
  ...
  useEffect(() => {
    ...
    async function ticker(_, metadata) {
      ...
      if (
        ...
        (await videoElement.playbackRate()) === 1 &&
        document.hasFocus()
      ) {
    ...
    videoElement.element()?.addEventListener?.("seeked", seekDetector);
    return () => {
      videoElement.element()?.removeEventListener?.("seeked", seekDetector);
    };
